...
const AdminTrips = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const driverList = Array.isArray(driverRes)
        ? driverRes
        : driverRes?.users || [];
  ...
  const pendingOrders = useMemo(() => {
    const tripOrderIds = new Set(
      trips
        .map((trip) => trip.order?._id || trip.order)
        .filter(Boolean)
        .map(String)
    );
    ...
    return orders.filter(
      (order) =>
        ['approved', 'assigned'].includes(order.status) &&
        !tripOrderIds.has(String(order._id))
    );
  }, [orders, trips]);

  const selectedOrder =
    pendingOrders.find((order) => order._id === form.order) || null;

  const selectedTruck =
    trucks.find((truck) => truck._id === form.truck) || null;

  const selectedDriver =
    drivers.find((driver) => driver._id === form.driver) || null;
      ...
      <div className="assign-layout">
        <div className="dark-card" style={{ flex: 1 }}>
          ...
          >
            ...
            {selectedOrder && (
              <div className="users-note">
                Route: {selectedOrder.pickupLocation} →{' '}
